...
if ($eh_admin && $_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['acao']) && $_POST['acao'] === 'cadastrar') {
    ...
    if (!validarCSRF($_POST['csrf_token'] ?? '')) {
        ...
        if (strlen($nome) < 3)                       $erros[] = 'Nome do sensor deve ter ao menos 3 caracteres.';
        if (!in_array($tipo, $tipos_validos))        $erros[] = 'Tipo de sensor inválido.';
        if (!in_array($status, ['ativo','inativo'])) $erros[] = 'Status inválido.';
...
<?php if (!empty($erros)): ?>
    <div style="color:red;">
        <ul><?php foreach ($erros as $e): ?><li><?php echo htmlspecialchars($e); ?></li><?php endforeach; ?></ul>
    </div>
<?php endif; ?>
...
<form method="POST">
    ...
    <label>Nome do sensor: <input type="text" name="nome" maxlength="100" required placeholder="Ex: Sensor Galpão A"></label><br><br>
    ...
    <label>Localização: <input type="text" name="localizacao" maxlength="150" placeholder="Ex: Setor B, linha 3"></label><br><br>
    ...
    <table border="1" cellpadding="5" cellspacing="0">
        <thead>
            <tr>
                ...
                <th>Status</th>
                <th>Cadastrado em</th>
                <?php if ($eh_admin): ?><th>Ação</th><?php endif; ?>
            </tr>
        </thead>
        <tbody>
